/**
 * ESP32 Polling-based TTS Audio Player - PSRAM Enhanced Version
 * 基于轮询的TTS音频播放系统 - PSRAM增强版本
 * ESP32-S3-WROOM-1-N16R8 with PSRAM support
 */
...
/* Audio Configuration - 提升最大音频文件大小 */
#define SAMPLE_RATE            48000        // 使用48kHz，与原项目一致
#define BITS_PER_SAMPLE        16
#define DMA_BUF_LEN            1024
#define DMA_BUF_COUNT          8

/* Audio buffer configuration - 利用PSRAM增大缓冲区 */
#define MAX_AUDIO_SIZE         (8 * 1024 * 1024)  // 提升到8MB，利用PSRAM
#define DOWNLOAD_CHUNK_SIZE    (64 * 1024)        // 增大下载块大小
#define POLL_INTERVAL_MS       2000       

static const char *TAG = "ESP32_POLLING_AUDIO_PSRAM";
...
static void* psram_malloc(size_t size) {
    ...
    if (ptr) {
        ESP_LOGD(TAG, "PSRAM allocated %d bytes at %p", size, ptr);
    } else {
        ESP_LOGW(TAG, "PSRAM allocation failed for %d bytes, fallback to regular RAM", size);
        ptr = malloc(size);  // 回退到常规RAM
    }
    return ptr;
}
...
static void* psram_realloc(void* ptr, size_t size) {
    ...
    if (new_ptr) {
        ESP_LOGD(TAG, "PSRAM reallocated to %d bytes at %p", size, new_ptr);
    } else {
        ESP_LOGW(TAG, "PSRAM reallocation failed for %d bytes, fallback to regular RAM", size);
        new_ptr = realloc(ptr, size);  // 回退到常规RAM
    }
    return new_ptr;
}
...
static void psram_free(void* ptr) {
    if (ptr) {
        heap_caps_free(ptr);
        ESP_LOGD(TAG, "PSRAM freed at %p", ptr);
    }
}
            ...
            if (!esp_http_client_is_chunked_response(evt->client)) {
                ...
                if (download_state->size + evt->data_len > download_state->capacity) {
                    ...
                    if (new_capacity > MAX_AUDIO_SIZE) {
                        ...
                        if (!new_buffer) {
                            ESP_LOGE(TAG, "Failed to reallocate download buffer in PSRAM");
                            return ESP_FAIL;
                        }
                        download_state->buffer = new_buffer;
                        download_state->capacity = new_capacity;
                        ESP_LOGD(TAG, "Expanded PSRAM buffer to %d bytes", new_capacity);
                    }
                }
...
static esp_err_t download_pcm_audio(const char *audio_id) {
    ...
    if (audio_state.audio_buffer) {
        psram_free(audio_state.audio_buffer);
    ...
    if (!initial_buffer) {
        ESP_LOGE(TAG, "Failed to allocate initial download buffer in PSRAM");
        return ESP_FAIL;
    }
    ...
    if (err == ESP_OK) {
        ...
        if (status_code == 200 && download_state.size > 0) {
            ...
            psram_free(download_state.buffer);
            err = ESP_FAIL;
        }
    } else {
        ...
        if (audio_state.is_playing && audio_state.has_audio) {
            ...
            if (remaining == 0) {
                ...
                if (audio_state.audio_buffer) {
                    psram_free(audio_state.audio_buffer);
                    audio_state.audio_buffer = NULL;
                    ESP_LOGI(TAG, "Audio buffer freed, heap: %d bytes, PSRAM: %d bytes", 
                             esp_get_free_heap_size(),
                             heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
                }
                continue;
